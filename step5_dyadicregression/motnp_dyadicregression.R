#### Information ####
# script takes data input from edge weight estimation (input = 1000 draws for each of 4 chains per dyad posterior distribution)
...
#### Read in data ####
df_agg <- read_csv('data_processed/motnp_bayesian_allpairwiseevents_splitbygrouptype_22.01.13.csv')
...
#edge_samples <- read_csv('data_processed/motnp_bayesian_edgedistributions_a2.b2_22.02.07.csv')
#saveRDS(edge_samples, 'data_processed/motnp_bayesian_edgedistributions_a2.b2_22.02.07.rds')
edge_samples <- readRDS('data_processed/motnp_bayesian_edgedistributions_a2.b2_22.02.07.rds') %>% 
  select(-`1.lp__`)
...
df_agg$age_diff_std <- standardize(df_agg$age_diff)
...
# load model:
model_dyadic <- stan_model('models/dyadic_regression_jhart96_hkmplaying_22.05.23.stan')
...
#### Fit model ####
model_data <- list(
  N = nrow(df_agg),                             # Number of dyads
  K = length(unique(c(df_agg$node_1, df_agg$node_2))),  # Number of nodes
  logit_edge_mu = logit_edge_mu,                # Sample means of edge weights
  logit_edge_cov = logit_edge_cov,              # Sample covariance of edge weights
  dyad_type = as.integer(df_agg$dyad_type),     # Integer dyad types corresponding to ages of pairs
