...
save.image('step5_dyadicregression/anp_dyadic_simulation_diffmean_partialpool_dyadrandom.RData')
...
#################### Age difference ####################
load('step5_dyadicregression/anp_dyadic_simulation_dataprep.RData')
dev.off()
pdf('step5_dyadicregression/simulate_dyadic_anp_diff_partialpool_dyadrandom.pdf')
...
save.image('step5_dyadicregression/anp_dyadic_simulation_agediff_partialpool_dyadrandom.RData')

#### fit dyadic regression ####
#load('step5_dyadicregression/anp_dyadic_simulation_agediff_partialpool_dyadrandom.RData')
...
dyadic_regression <- stan_model('models/dyadic_regression_anp_agediff_dyadrandom.stan')
#dyadic_regression <- cmdstan_model('models/dyadic_regression_anp_agediff_dyadrandom.stan')
...
# #################### Age mean ####################
# rm(list = ls()[! ls() %in% c('traceplot','summary','sim_dat','sim_diff','sim_int','sim_mean','sim_node_effect','sim_sigma','sim_window_effect','node_data','dyad_data','n_chains','n_data','n_dyads','n_nodes','n_samples','n_windows','edge_summary','get_full_predictions','get_mean_predictions','logit_edge_draws_mu','logit_edge_draws_sd')])
# 
# colnames(edge_summary)
# edge_summary <- edge_summary[,1:42] # remove all predictions from previous model
# str(edge_summary)
# 
# dev.off()
# pdf('step5_dyadicregression/simulate_dyadic_anp_mean_partialpool.pdf')
# 
# #### prior predictive check ####
# n <- 100
# beta_age_mean <- rnorm(n, 0, 0.8)
# intercept <- rnorm(n, 0, 2)
# window <- rnorm(n, 0, 1)
# nodes <- rnorm(n*2, 0, 1)
# plot(NULL, las = 1, xlab = 'mean age', ylab = 'logit edge weight (standardised)',
...
#   y <- rep(NA, length(x))
#   for(j in 1:length(x)){
#     y[j] <- intercept[i] + beta_age_mean[i]*x[j] + window[j] + nodes[j] + nodes[n+j]
#   }
#   lines(x = x, y = y, col = rgb(0,0,1,0.4))
# }
...
# ## print progress marker
# print('prior predictive complete')
# save.image('step5_dyadicregression/anp_dyadic_simulation_agemean_partialpool.RData')
# 
# #### fit dyadic regression ####
# #load('step5_dyadicregression/anp_dyadic_simulation_agemean_partialpool.RData')
# 
# ## create data list
# dyad_data <- list(
#   # global data size
...
#   mu_plot <- rep(NA, n_dyads)
#   for(k in 1:n_dyads){
#     mu_plot[k] <- intercept[j] + b_mean[j]*dyad_data$age_mean[k] + mm_nodes[j,dyad_data$node_1[k]] + mm_nodes[j,dyad_data$node_2[k]]# + rand_window[j,dyad_data$window[k]]# + rand_dyad[j,dyad_data$dyad_id[k]]
#   }
#   
#   sigma_plot <- dyad_data$logit_edge_sd + diag(rep(sigma[j], n_data))
#   norm <- rnorm(1000, mu_plot, sigma_plot)
...
#              alpha = 0.2
#   )+
#   scale_x_continuous('mean age of dyad members')+
#   scale_y_continuous('mean estimated edge weight')+
#   facet_wrap(. ~ window)+
#   theme(axis.text = element_text(size = 14),
#         legend.position = 'bottom', #c(0.8,0.2),
...
#     
#     for(draw in 1:nrow(mu)){
#       mu[draw, dyad] <- intcp[draw] + b_mean[draw] * prediction_df$age_mean_std[dyad] +  node_1[draw] + node_2[draw]# + window_effect[draw]# + dyad_effect[draw]
#       #
#     }
#   }
#   return(mu)
...
# 
# ## calculate contrasts: mean age increases by 1 stdv, standardised scale
