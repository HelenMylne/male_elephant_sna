...
# load('anp_edgecalculations/anpshort5_edgeweights_conditionalprior.RData') # load('anp_edgecalculations/excluding_under10s/anpshort5_edgeweights_conditionalprior.RData')
# edge_summary <- summary %>%
#   mutate(dyad_random = sample(1:n_dyads, replace = F))
# node_data <- nodes
# rm(counts_df, edge_binary, edgelist, edges, fit_edges_anp1, cdf_1, periods, n_windows, make_edgelist, plot_network_threshold_anp, summary, nodes) ; gc()
# 
# # load('step5_dyadicregression/anpshort_dyadicregression_dataimported.RData')
# edge_summary <- edge_summary %>%
#   rename(window = period) %>%
#   mutate(dyad_random = as.integer(as.factor(dyad_random)))
# 
...
# str(edge_summary)
# edge_summary$mu <- sim_int + edge_summary$age_diff_std * sim_diff + edge_summary$age_mean_std * sim_mean + sim_node_effect[edge_summary$node_1_random] + sim_node_effect[edge_summary$node_2_random] # simulate mean centrality on unstandardised scale
# ggplot(edge_summary)+
...
# save.image('step5_dyadicregression/anp_dyadic_simulation_1window.RData')
# 
# #### plot raw data ####
# ggplot()+
...
# 
# save.image('step5_dyadicregression/anp_dyadic_simulation_dataprep_1window.RData')
# dev.off()
# 
#### prior predictive check ####
load('step5_dyadicregression/anp_dyadic_simulation_dataprep_1window.RData')
pdf('step5_dyadicregression/simulate_dyadic_anp_diffmean_checkoutputs.pdf')

n <- 100
beta_age_mean <- rnorm(n, 0, 0.8)
beta_age_diff <- rnorm(n, 0, 0.8)
...
par(mfrow = c(3,3), mai = c(0.2,0.2,0.2,0.2))
for(mean_age in seq(min(edge_summary$age_mean_std), max(edge_summary$age_mean_std),
                    length.out = 9)){
...
rm(n, beta_age_diff,tau_sigma_raw,mu_mm,rand_mm,tau_mm,
   raw_sigma,node_mean,tau_sigma,node_sigma,
   # mu_window,rand_window,sigma_window,window_random_effect,
   mm_nodes, i, y, j, x) ; gc()
...
#load('step5_dyadicregression/anp_dyadic_simulation_diffmean.RData')
...
dyadic_regression <- stan_model('models/dyadic_regression_anp_diffmean.stan')
...
for (i in 1:100) {
  ...
  for(k in 1:n_dyads){
    mu_plot[k] <- b_diff[j]*dyad_data$age_diff[k] + b_mean[j]*dyad_data$age_mean[k] + mm_nodes[j,dyad_data$node_1[k]] + mm_nodes[j,dyad_data$node_2[k]]# + rand_window[j,dyad_data$window[k]]
  }

  sigma_plot <- dyad_data$logit_edge_sd + rep(sigma[j], n_data)
...
dev.off()
pdf('step5_dyadicregression/simulate_dyadic_anp_diffmean_predictions.pdf')
  ...
  for(dyad in 1:ncol(mu)){
    ...
    for(draw in 1:nrow(mu)){
      mu[draw, dyad] <- b_diff[draw] * prediction_df$age_diff_std[dyad] +
        b_mean[draw] * prediction_df$age_mean_std[dyad] +
...
## predict full distribution
fake_full <- get_full_predictions(mu_matrix = fake_mean,
                                  sigma = draws$sigma,
                                  sd = logit_edge_draws_sd)
warnings()
