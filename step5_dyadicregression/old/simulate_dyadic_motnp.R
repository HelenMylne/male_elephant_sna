...
# test <- diag(rep(1, ncol(logit_edge_draws_cov)),
#              ncol = ncol(logit_edge_draws_cov))
# logit_edge_draws_cov_2 <- logit_edge_draws_cov + test
# cholesky <- chol(logit_edge_draws_cov_2)
# check <- cholesky %*% t(cholesky)
# which(check != logit_edge_draws_cov_2)

## decompose covariance matrix with SVD for smaller memory consumption
cov_svd <- svd(logit_edge_draws_cov)
cov_svd$d # vector of scalar values, of decreasing size, length n_dyads
d <- diag(x = cov_svd$d) # diagonal matrix with values cov_svd$d
cov_svd$u # matrix n_dyads * n_dyads
cov_svd$v # matrix n_dyads * n_dyads
vt <- t(cov_svd$v)

## determine rank-level for low-rank approximation of covariance matrix: choose k such that the sum of the top k singular values is at least c times as big as the sum of the other singular values, where c is a domain-dependent constant (like 10, say)
hist(cov_svd$d)
constant <- 10
k <- NA
for(i in 1:n_dyads){
  if(is.na(k) == TRUE){
    if(sum(cov_svd$d[1:i] > constant * sum(cov_svd$d[i:n_dyads]))){
      k <- as.numeric(i)
    }
  }
}

## create low-rank approximation of covariance matrix from SVD
uk <- cov_svd$u[1:k,1:k]
dk <- d[1:k,1:k]
vtk <- vt[1:k,1:k]
compressed_cov <- uk %*% dk %*% vtk
print(paste0('covariance matrix = ', dim(logit_edge_draws_cov)[1], ' x ', dim(logit_edge_draws_cov)[2],
             ' and size = ', format(object.size(logit_edge_draws_cov), units = 'Kb'),
             '; compressed matrix = ',dim(compressed_cov)[1], ' x ', dim(compressed_cov)[2],
             ' and size = ', format(object.size(compressed_cov), units = 'Kb')))
...
dyad_data <- list(
  ...
  logit_edge_cov = compressed_cov,    # compressed covariance of logit edge weights
