...
# beta_age_min <- rnorm(n, 0, 2.5)
# intercept <- rnorm(n, 0, 1)
# min_dirichlet <- rdirichlet(n, rep(1, n_age_cat))
# mm_nodes <- rnorm(n, 0, rexp(1, 1))
# plot(NULL, las = 1, xlab = 'minimum age categeory', ylab = 'logit edge weight (standardised)',
#      ylim = c(-8,10),#c(min(logit_edge_draws_mu)-5, max(logit_edge_draws_mu)+5),
#      xlim = c(1,n_age_cat))
# abline(h = -2, #min(logit_edge_draws_mu),
#        lty = 2) ;
# abline(h = 3.5, #max(logit_edge_draws_mu),
#        lty = 2)
# x <- 1:n_age_cat
# for(i in 1:n){
#   y <- rep(NA, length(x))
