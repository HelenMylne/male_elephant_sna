...
beta_age_min <- rnorm(n,0,5)
min_dirichlet <- rdirichlet(n, rep(1, n_age_cat))
min_dirichlet <- cbind(rep(0, n), min_dirichlet)
...
for(bounds in c(100, 50, 20, 5)){
  ...
  plot(NULL, las = 1, xlab = 'minimum age categeory', 
  ...
  for(i in 1:n){
    ...
    for(j in 1:length(x)){
      y[j] <- intercept[i] + 
        beta_age_min[i]*sum(min_dirichlet[i,][1:x[j]]) + 
...
fit_dyadreg_sim <- sampling(dyadic_regression,
                            data = dyad_data,
                            iter = n_samples*5, warmup = n_samples*4,
...
# plot(density(as.numeric(sim_dat[1, ])), ylim = c(0, 1), col = rgb(0, 0, 0, 0.25), las = 1
...
#       mm_nodes[j,dyad_data$node_1[k]] + mm_nodes[j,dyad_data$node_2[k]]
#   }
  ...
  for(l in c(1:n_chains)){
    ...
    for(k in 1:n_dyads){
      mu_plot[l,k] <- intercept[j+n] + 
        b_min[j+n]*sum(delta_j_min[j,(1:dyad_data$age_min_cat[k])]) + 
...
for(i in 1:nrow(sim)){
  sim$mu_pred_basic[i] <- int_mu +
    b_mu * sum(dj_mu[1:sim$age_cat_min[i]]) +
...
# save.image('step5_dyadicregression/motnp_dyadic_simulation_agemin.RData')
# # load('step5_dyadicregression/motnp_dyadic_simulation_agemin.RData')
# # rm(list = ls()[! ls() %in% c('predictions_full', 'predictions_mean','sim_min')]) ; gc()
# 
# ## add progress marker
# print('predictions complete')
# 
...
#   contrasts$num_years_change[i] <- contrasts$alt_min_lwr[i] - contrasts$org_min_lwr[i]
...
#   contrasts$lwr_contrast[i]  <- quantile(contrast_matrix, prob = 0.025)
#   contrasts$upr_contrast[i]  <- quantile(contrast_matrix, prob = 0.975)
#   contrasts$num_years_change[i] <- contrasts$alt_max_lwr[i] - contrasts$org_max_lwr[i]
#   contrasts$mean_per_year[i] <- mean(contrast_matrix) / contrasts$num_years_change[i]
# }
# 
# ## add progress marker
...
# ## plot together
# load('step5_dyadicregression/motnp_dyadic_simulation_agemin.RData')
# rm(list = ls()[! ls() %in% c('min_plot', 'max_plot')])
# 
# (min_plot + max_plot) +
#   plot_annotation(tag_levels = 'a') +
#   plot_layout(guides = "collect")
# ggsave(plot = last_plot(), device = 'png',
#        filename = 'motnp_dyadicsimulation_minmax.png',
#        path = 'step5_dyadicregression/',
#        height = 1600, width = 2700, units = 'px')
# 
# dev.off()
