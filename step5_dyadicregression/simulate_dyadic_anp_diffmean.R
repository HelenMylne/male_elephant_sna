...
pred <- data.frame(age_diff_std = rep(rep(unique(dyad_data$age_diff),
                                          each = age_max_length),
                                      n_chains*n_samples),
                   age_diff = rep(rep(unique(edge_summary$age_diff),
                                      each = age_max_length),
                                  n_chains*n_samples),
                   intcp = rep(intercept,
                               each = age_max_length*age_diff_length),
...
#   mutate(age_diff_new_std = (age_diff_new - mean_agediff)/stdv_agediff)
pred_new_diff <- pred_new %>%
  mutate(age_diff_new_std = age_diff_org_std + 1) %>%
  mutate(age_diff_new = age_diff_new_std * stdv_agediff + mean_age)
...
fake_data <- expand.grid(window = 1:n_windows,
                         ...
                         age_diff = min(node_data$agediff):max(node_data$agediff)) %>% 
  left_join(distinct(edge_summary[,c('dyad_random','node_1','node_2')]),
            by = 'dyad_random') %>% 
  mutate(age_diff_std = (age_diff - mean_age) / stdv_age)
...
pred <- data.frame(age_mean_std = rep(rep(unique(dyad_data$age_mean),
                                          each = age_max_length),
                                      n_chains*n_samples),
                   age_mean = rep(rep(unique(edge_summary$age_mean),
                                      each = age_max_length),
                                  n_chains*n_samples),
                   intcp = rep(intercept,
                               each = age_max_length*age_mean_length),
...
pred_new_mean <- pred_new %>%
  ...
  mutate(age_mean_new = age_mean_new_std * stdv_agemean + mean_age)
