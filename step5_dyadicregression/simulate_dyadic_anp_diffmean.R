...
edge_summary <- edge_summary %>%
  ...
  mutate(dyad_window = paste0(dyad_random, '_', window))
...
save.image('step5_dyadicregression/anp_dyadic_simulation_agediff_nointercept.RData')
...
save.image('step5_dyadicregression/anp_dyadic_simulation_dataprep.RData')

# #################### Age difference ####################
# load('step5_dyadicregression/anp_dyadic_simulation_dataprep_9windows.RData')
# # dev.off()
# pdf('step5_dyadicregression/simulate_dyadic_anp_diff_nointercept.pdf')
# 
...
# #load('step5_dyadicregression/anp_dyadic_simulation_agediff_nointercept.RData')
# 
# ## create data list
# dyad_data <- list(
#   # global data size
...
# dyadic_regression <- stan_model('models/dyadic_regression_anp_agediff_nointercept.stan')
# #dyadic_regression <- cmdstan_model('models/dyadic_regression_anp_agediff_nointercept.stan')
# 
# ## print progress marker
...
#   pivot_longer(cols = c('beta_age_diff','sigma'),
...
#   for(k in 1:n_dyads){
#     mu_plot[k] <- b_diff[j]*dyad_data$age_diff[k] + mm_nodes[j,dyad_data$node_1[k]] + mm_nodes[j,dyad_data$node_2[k]] + rand_window[j,dyad_data$window[k]]
#   }
# 
...
#   mu <- matrix(NA, nrow = (n_samples*n_chains), ncol = nrow(prediction_df),
#                dimnames = list(1:(n_samples*n_chains),
#                                paste0('dyad',prediction_df$dyad_window)))
# 
...
#       mu[draw, dyad] <- b_diff[draw] * prediction_df$age_diff_std[dyad] +
#         node_1[draw] + node_2[draw] + window_effect[draw]
#     }
#   }
#   return(mu)
# }
...
# 
#     for(draw in 1:nrow(mu)){
#       mu[draw, dyad] <- b_diff[draw] * prediction_df$age_diff_std[dyad] +  node_1[draw] + node_2[draw] + window_effect[draw]
#     }
#   }
#   return(mu)
# }
...
# # load('step5_dyadicregression/anp_dyadic_simulation_agediff_nointercept.RData')
# rm(list = ls()[!ls() %in% c('traceplot','mm_nodes','mm_nodes_long','rand_window','rand_window_long','edge_summary')]) ; gc()
# save.image('step5_dyadicregression/anp_diff_correlatedrandom.RData')
# # load('step5_dyadicregression/anp_correlatedrandom.RData')
# 
# mm_nodes_long %>% traceplot()
# rand_window_long %>% traceplot()
# 
# n_windows <- length(unique(edge_summary$window))
# n_plots <- 25
# 
# pdf('step5_dyadicregression/simulate_dyadic_anp_correlatedrandom_correlations.pdf')
# par(mfrow = c(n_plots/5, 5),
#     mai = c(0.2,0.2,0.2,0.2))
# for(window_id in 1:n_windows){
#   window_summary <- edge_summary %>% filter(window == window_id)
#   nodes_select <- unique(c(window_summary$node_1_random, window_summary$node_2_random))
#   mm_window <- mm_nodes[,nodes_select]
#   # mm_window_long <- mm_nodes_long %>% 
#   #   filter(as.numeric(parameter) %in% nodes_select)
#   rand_window_id <- rand_window[,window_id]
#   # rand_window_long_id <- rand_window_long %>% 
#   #   filter(as.numeric(parameter) == window_id)
#   
#   nodes_select <- sample(colnames(mm_window), n_plots, replace = F)
#   
#   mm_plot <- mm_window[,nodes_select]
#   
#   for(i in 1:n_plots){
#     plot(rand_window_id, mm_plot[,i])
#   }
#   
# }
# dev.off()
...
#################### mean of dyad age ####################
load('step5_dyadicregression/anp_dyadic_simulation_dataprep_1window.RData')
# dev.off()
pdf('step5_dyadicregression/simulate_dyadic_anp_mean.pdf')
...
mu_mm <- rnorm(n, 0,0.5)#logit(0.05), 0.5)
rand_mm <- rnorm(n, 0, 1)
tau_mm <- rnorm(n, 0, 0.5) # rcauchy(n, 0, 0.5)
raw_sigma <- rnorm(n, 0, 1)
...
plot(NULL, las = 1, xlab = 'mean of dyad age', ylab = 'logit edge weight (standardised)',
     ylim = c(min(logit_edge_draws_mu)-20, max(logit_edge_draws_mu)+20),
...
rm(n, beta_age_mean,tau_sigma_raw,mu_mm,rand_mm,tau_mm,raw_sigma,node_mean,tau_sigma,node_sigma,mu_window,rand_window,sigma_window,window_random_effect,mm_nodes, i, y, j, x) ; gc()
...
# fit_dyadreg_simmean <- vb(dyadic_regression, data = dyad_data)
...
parameters <- data.frame(beta_age_mean = b_mean,
  ...
  mutate(chain = rep(1:n_chains, each = n_samples),
         position = rep(1:n_samples, n_chains)) %>%
  pivot_longer(cols = c('beta_age_mean','sigma'),
               names_to = 'parameter', values_to = 'slope_draw')
...
for (i in 1:100) {
  ...
  mu_plot <- rep(NA, n_dyads)
  for(k in 1:n_dyads){
    mu_plot[k] <- b_mean[j]*dyad_data$age_mean[k] + mm_nodes[j,dyad_data$node_1[k]] + mm_nodes[j,dyad_data$node_2[k]]# + rand_window[j,dyad_data$window[k]]
  }
...
get_mean_predictions <- function(prediction_df, draws_list){
  mu <- matrix(NA, nrow = (n_samples*n_chains), ncol = nrow(prediction_df),
               dimnames = list(1:(n_samples*n_chains),
                               paste0('dyad',prediction_df$dyad_id)))
  ...
  for(dyad in 1:ncol(mu)){
    ...
    for(draw in 1:nrow(mu)){
      mu[draw, dyad] <- b_mean[draw] * prediction_df$age_mean_std[dyad] +
        node_1[draw] + node_2[draw]# + window_effect[draw]
    }
  }
  return(mu)
...
# ## summarise
# pred_summary <- pred %>%
#   group_by(age_mean_std, age_mean_std) %>%
#   mutate(pred_lwr = quantile(pred_mu, probs = 0.025),
#          pred_mean = mean(pred_mu),
#          pred_upr = quantile(pred_mu, probs = 0.975),
#          pred_unstd_lwr = quantile(pred_unstd, probs = 0.025),
...
#          pred_unstd_upr = quantile(pred_unstd, probs = 0.975)) %>%
#   ungroup() %>%
#   select(age_mean_std, age_mean_std,
#          pred_lwr, pred_mean, pred_upr,
#          pred_unstd_lwr, pred_unstd_mean, pred_unstd_upr) %>%
#   distinct()
...
ggplot()+
  ...
  scale_x_continuous('mean of dyad age')+
  scale_y_continuous('mean estimated edge weight')+
  # facet_wrap(. ~ window)+
  theme(axis.text = element_text(size = 14),
        legend.position = 'bottom', #c(0.8,0.2),
...
#   mutate(age_mean_new_std = (age_mean_new - mean(edge_summary$age_mean))/sd(edge_summary$age_mean))
pred_new_mean <- pred_new %>%
  mutate(age_mean_new_std = age_mean_org_std + 1) %>%
  mutate(age_mean_new = age_mean_new_std * sd(edge_summary$age_mean) + mean(edge_summary$age_mean))
...
get_mean_predictions <- function(prediction_df, draws_list, new){
  ...
  for(dyad in 1:ncol(mu)){
    ...
    for(draw in 1:nrow(mu)){
      mu[draw, dyad] <- b_mean[draw] * prediction_df$age_mean_std[dyad] +  node_1[draw] + node_2[draw] + window_effect[draw]
    }
  }
  return(mu)
}
...
## calculate contrasts: mean of dyad age increases by 1 stdv, standardised scale
...
fake_data <- expand.grid(window = 1:n_windows,
                         dyad_random = unique(edge_summary$dyad_random),
                         age_mean = min(edge_summary$age_mean):max(edge_summary$age_mean)) %>%
  left_join(distinct(edge_summary[,c('dyad_random','node_1','node_2')]),
            by = 'dyad_random') %>%
  mutate(age_mean_std = (age_mean - mean(edge_summary$age_mean)) / sd(edge_summary$age_mean))
...
save.image('step5_dyadicregression/anp_dyadic_simulation_agemean.RData')
dev.off()
print('mean of dyad age complete')
