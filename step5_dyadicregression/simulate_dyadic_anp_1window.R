...
dev.off()
pdf('step5_dyadicregression/simulate_dyadic_anp_1window.pdf')
...
rm(intercept, beta_age_diff, tau_sigma_raw, mu_mm, rand_mm, tau_mm, raw_sigma, node_mean, tau_sigma, node_sigma, mm_nodes, i, j, n, x, y) ; gc()
...
dyadic_regression <- stan_model('models/dyadic_regression_anp_1window.stan')
...
rm(i,j,k,l,n,norm,mu_plot,sigma_plot) ; gc()
save.image('step5_dyadicregression/anp_dyadic_simulation_1window.RData')
...
get_mean_predictions <- function(prediction_df, draws_list){
  mu <- matrix(NA, nrow = length(draws_list$beta_age_diff), ncol = nrow(prediction_df),
               dimnames = list(1:length(draws_list$beta_age_diff),
  ...
  for(dyad in 1:ncol(mu)){
    ...
    for(draw in 1:nrow(mu)){
      mu[draw, dyad] <- #intcp[draw] + 
        b_diff[draw] * prediction_df$age_diff_std[dyad] +
...
get_mean_predictions <- function(prediction_df, draws_list, new){
  mu <- matrix(NA, nrow = length(draws_list$beta_age_diff),
               ncol = nrow(prediction_df),
               dimnames = list(1:length(draws_list$beta_age_diff),
                               paste0('dyad',prediction_df$dyad_random)))
  ...
  for(dyad in 1:ncol(mu)){
    ...
    for(draw in 1:nrow(mu)){
      mu[draw, dyad] <- #intcp[draw] + 
        b_diff[draw] * prediction_df$age_diff_std[dyad] +  node_1[draw] + node_2[draw]
    }
  }
  return(mu)
}
...
fake_data <- expand.grid(dyad_random = unique(edge_summary$dyad_random),
  ...
  left_join(distinct(edge_summary[,c('dyad_random','node_1_random','node_2_random')]),
            by = 'dyad_random') %>%
  mutate(age_diff_std = (age_diff - mean(edge_summary$age_diff)) / sd(edge_summary$age_diff))

## predict mean values
fake_mean <- get_mean_predictions(prediction_df = fake_data,
                                  draws_list = draws, new = NULL)
length(which(is.na(fake_mean) == TRUE))
print('mean values extracted')

## predict full distribution
fake_full <- get_full_predictions(mu_matrix = fake_mean, sigma = draws$tau_sigma)
warnings()
...
save.image('step5_dyadicregression/anp_dyadic_simulation_plottingfake_1window.RData')
...
ggsave(plot = diff_overall, device = 'png',
       filename = 'anp_dyadic_simulation_diffageeffect_overall_1window.png',
