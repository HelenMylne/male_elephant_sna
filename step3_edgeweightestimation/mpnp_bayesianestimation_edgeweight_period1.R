...
# add pdf output file
pdf(file = '../outputs/mpnp1_bisonr_edgeweight_cv.pdf')
...
## coefficient of variation of edge weights (aka social differentiation) ####
# add pdf output file
pdf(file = '../outputs/mpnp2_bisonr_edgeweight_cv.pdf')
...
## coefficient of variation of edge weights (aka social differentiation) ####
# add pdf output file
pdf(file = '../outputs/mpnp3_bisonr_edgeweight_cv.pdf')
...
## coefficient of variation of edge weights (aka social differentiation) ####
# add pdf output file
pdf(file = '../outputs/mpnp4_bisonr_edgeweight_cv.pdf')
...
## coefficient of variation of edge weights (aka social differentiation) ####
# add pdf output file
pdf(file = '../outputs/mpnp5_bisonr_edgeweight_cv.pdf')
...
## create data list ####
# create file of output graphs
pdf('../outputs/mpnp6_networkplots.pdf', width = 10, height = 10)

counts_df6 <- read_csv('../data_processed/mpnp_period6_pairwiseevents.csv')
str(counts_df6)

### load in ages 
mpnp6_ages <- readRDS('../data_processed/mpnp6_ageestimates_mcmcoutput.rds') %>%
  pivot_longer(cols = everything(), names_to = 'id', values_to = 'age')

### filter counts data frame down to males only
counts_df6 <- counts_df6 %>% 
  filter(id_1 %in% unique(mpnp6_ages$id)) %>% 
  filter(id_2 %in% unique(mpnp6_ages$id))

# reassign dyad numbers to remove gaps
counts_df6$node_1_males <- as.integer(as.factor(counts_df6$node_1))
counts_df6$node_2_males <- as.integer(as.factor(counts_df6$node_2))+1

### standardise dyad_id
counts_df6$dyad_males <- as.integer(as.factor(counts_df6$dyad_id))

## edge weights ####
### create data frame for edge weight model
counts_df6_model <- counts_df6[, c('node_1_males','node_2_males','together','count_dyad')] %>% distinct()
colnames(counts_df6_model) <- c('node_1_id','node_2_id','event','duration')
...
### run edge weight model
mpnp6_edge_weights <- bison_model(
  ( event | duration ) ~ dyad(node_1_id, node_2_id), 
  data = counts_df6_model, 
  model_type = "binary",
  #mc_cores = 4,
  priors = priors
)
...
### run diagnostic plots
plot_trace(mpnp6_edge_weights, par_ids = 2)
plot_predictions(mpnp6_edge_weights, num_draws = 20, type = "density")
plot_predictions(mpnp6_edge_weights, num_draws = 20, type = "point")

### extract edge weight summaries
edgelist <- get_edgelist(mpnp6_edge_weights, ci = 0.9, transform = TRUE)
plot(density(edgelist$median))
summary(mpnp6_edge_weights)
...
### compare null model with fitted model -- bisonR comparison
model_comparison(list(non_random_model = mpnp6_edge_weights, random_model = mpnp6_edges_null))
...
model_averaging(models = list(non_random_model = mpnp6_edge_weights, random_model = mpnp6_edges_null))
...
for(i in 1:nrow(nodes)){
  ...
  nodes$age[i] <- mean(x$age)
  if(nodes$bull[i] != 'M99'){
    y <- counts_df6[counts_df6$id_1 == nodes$bull[i], c('id_1','count_1')]
    nodes$sightings[i] <- y[1,2]
  }
}
nodes$sightings <- as.numeric(nodes$sightings)
...
### plot network
plot_network_threshold(mpnp6_edge_weights, lwd = 2, ci = 0.9, threshold = 0.2,
                       vertex.label.color1 = NA, edge.color1 = rgb(0,0,0,0.25),
                       vertex.label.color2 = 'black', vertex.color2 = nodes$age,
                       vertex.size2 = nodes$sightings, edge.color2 = 'black')
...
### plot network
plot_network_threshold2(obj = mpnp6_edge_weights, threshold = 0.2,
                        vertex.color2 = nodes$age, vertex.size2 = nodes$sightings)
...
draws <- as.data.frame(mpnp6_edge_weights$chain) %>% pivot_longer(cols = everything(), names_to = 'dyad_model', values_to = 'edge')
draws$dyad_id <- rep(counts_df$dyad_id, 4000) ## IS THIS RIGHT??
draws$weight <- gtools::inv.logit(draws$edge)
draws$draw <- rep(1:4000,  each = nrow(counts_df_model))
draws <- left_join(draws, dyads, by = 'dyad_id')
...
write_csv(subset_draws, '../data_processed/mpnp6_sampledyads_random_binary_vs_sri.csv')
...
write_csv(subset_draws, '../data_processed/mpnp6_sampledyads_sri0.2_binary_vs_sri.csv')
...
## coefficient of variation of edge weights (aka social differentiation) ####
# add pdf output file
pdf(file = '../outputs/mpnp6_bisonr_edgeweight_cv.pdf')

# extract cv for model
global_cv <- extract_metric(mpnp6_edge_weights, "global_cv")
head(global_cv)
hist(global_cv)
...
counts_df_model <- counts_df[,c('node_1_males','node_2_males','event_count','count_dyad','id_1','id_2','dyad_males')]
colnames(counts_df_model) <- c('node_1_id','node_2_id','event','duration','id_1','id_2','dyad_id')
ms_chain <- as.data.frame(mpnp6_edge_weights$chain)
colnames(ms_chain) <- counts_df_model$dyad_id
ms_chain <- pivot_longer(ms_chain, cols = everything(), names_to = 'dyad_id', values_to = 'draw')
ms_chain$chain_position <- rep(1:length(unique(ms_chain$dyad_id)), each = 4000)
ms_chain$draw <- LaplacesDemon::invlogit(ms_chain$draw)
...
ms_edgelist <- bisonR::get_edgelist(mpnp6_edge_weights)
quantile(ms_edgelist$median, 0.98)
...
## clean up ####
rm(list= ls()[!(ls() %in% c('mpnp6_edge_weights','mpnp6_edges_null',
                            'counts_df','counts_df_model',
                            'gbi_males','m_mat',
                            #'random_nodes',
                            'random_networks'))])
...
### write over saved workspace image -- only keep the things you might need later that take a long time to run
save.image('mpnp6_bisonr_edgescalculated.RData')
