...
#### long time windows ####
rm(list = ls()[!(ls() %in% c('ate','sightings','priors',
                             'model_averaging', 'plot_network_threshold','plot_network_threshold2'))])
...
for( time_window in 1:length(unique(counts_df_allwindows$period))){
  ...
  # save workspace image
  save.image(file = paste0('anp_edgecalculations/anplong',time_window,'_bisonr_edgescalculated.RData'))
  
  ### clear environment
  rm(list = ls()[!(ls() %in% c('counts_df_allwindows','priors','periods','windows','males','time_window',
                               'model_averaging', 'plot_network_threshold','plot_network_threshold2'))])
