...
transformed parameters {
  ...
  matrix[num_nodes, num_nodes] L_cov;
  L_cov = cholesky_decompose(centrality_cov + diag_matrix(rep_vector(sigma, num_nodes)));
 ...
 model {
  // priors
  intercept ~ normal(-10,3); //intercept ~ normal(0,1);
  delta ~ dirichlet(prior_age);
  beta_age  ~ normal(0,4);
  sigma ~ exponential(2);

  // likelihood
  centrality_mu ~ multi_normal_cholesky(predictor, L_cov);
}
