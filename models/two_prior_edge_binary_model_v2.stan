data{
    ...
    vector[2] prior_edge_mu; // vector of mean values for edge weight
    vector[2] prior_edge_sigma; // vector of standard deviation values for edge weight
...
real conditional_prior(real mu, int x) {
    if (seen_together[i] == 1) {
      return normal(mu | prior_edge_mu_1, prior_edge_sigma_1);
    } else {
      return normal(mu | prior_edge_mu_0, prior_edge_sigma_0);
    }
  }
      if (num_random > 0) {
