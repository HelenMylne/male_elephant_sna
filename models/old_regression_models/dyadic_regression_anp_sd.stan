...
transformed parameters {
  ...
  for(i in 1:num_data) {
    predictor[i] = intercept + beta_age_min * age_min[i] + beta_age_max * age_max[i] + mm_nodes[node_1[i]] + mm_nodes[node_2[i]] + window_random_effect[window[i]] + dyad_random_effect[dyad_id[i]];
  }
}

model {
  // intercept prior
  intercept ~ normal(0,3); // was previously trying to get -15 from normal(0,1), now trying to get -5 from normal(0,3) -- see if this works better
  // slope priors
  beta_age_max ~ normal(0.8,0.1); // beta_age_max ~ normal(0,1); -- altered just to see if it works when given almost exactly correct values and therefore it's just a priors problem
  beta_age_min ~ normal(-0.2,0.1); // beta_age_min ~ normal(0,1); -- altered just to see if it works when given almost exactly correct values and therefore it's just a priors problem
  // Cholesky variance
  sigma ~ exponential(2);
