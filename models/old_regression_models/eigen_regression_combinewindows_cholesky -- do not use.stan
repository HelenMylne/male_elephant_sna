...
transformed parameters {
  ...
  matrix[num_nodes_window1,num_nodes_window1] chol_cov1;
  chol_cov1 = cholesky_decompose(centrality_cov_1 + diag_matrix(rep_vector(sigma, num_nodes_window1)));
  matrix[num_nodes_window2,num_nodes_window2] chol_cov2;
  chol_cov2 = cholesky_decompose(centrality_cov_2 + diag_matrix(rep_vector(sigma, num_nodes_window2)));
  matrix[num_nodes_window3,num_nodes_window3] chol_cov3;
  chol_cov3 = cholesky_decompose(centrality_cov_3 + diag_matrix(rep_vector(sigma, num_nodes_window3)));
 ...
 model {
  ...
  sigma ~ exponential(2);
  intercept ~ normal(0,4); //intercept ~ normal(logit(0.05),2);
  rand_window ~ normal(0,1);
  rand_node ~ normal(0,1);

  // likelihood
  centrality_mu_1 ~ multi_normal_cholesky(predictor_window1, chol_cov1);
  centrality_mu_2 ~ multi_normal_cholesky(predictor_window2, chol_cov2);
  centrality_mu_3 ~ multi_normal_cholesky(predictor_window3, chol_cov3);
}
