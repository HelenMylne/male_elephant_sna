...
nodal_regression <- cmdstan_model('models/eigen_regression_intercept_standardiseall.stan')
...
## compare to raw data
sim$mean_predict <- sim$age_std * mean(params$beta_age) + mean(params$intercept)           # mean prediction = age * mean slope
...
plot(sim$mean_predict ~ sim$age_std, type = 'l', las = 1)        # plot against age
points(sim$mu_std ~ sim$age_std)                                 # add raw points -- VERY GOOD FIT
...
points(sim$mu ~ sim$age)                                 # add raw points -- gebnerally fits very well, struggles more when true values are 0
