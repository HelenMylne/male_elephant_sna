...
ggplot(sim)+
  ...
  theme_bw()
=======
sim_sort <- sim[order(sim$age),]
plot(sim_sort$mu_invlogit ~ sim_sort$age, ylim = c(0,1))
lines(sim_sort$mean_predict_invlogit ~ sim_sort$age)
polygon(y = c(sim_sort$lwr_invlogit, rev(sim_sort$upr_invlogit)),
        x = c(sim_sort$age, rev(sim_sort$age)),
        col = rgb(1,1,0,0.5), border = NA)
>>>>>>> Stashed changes

## full predictions
ages <- seq(min(sim$age_std), max(sim$age_std), length.out = 10)
pred_data <- data.frame(age_std = rep(ages, each = n_nodes*n_windows),
                        node = rep(rep(1:n_nodes, each = n_windows), length(ages)),
                        window = rep(1:2, n_nodes*length(ages)),
                        prediction = NA)
predictions <- pred_data
mu <- params$beta_age[1]*predictions$age_std + params$intercept[1] + as.numeric(rand_window[1,predictions$window]) + as.numeric(rand_node[1,predictions$node])
#sigma <- sim_cent_cov + diag(rep(params$sigma[i], n_data))
predictions$prediction <- mu #MASS::mvrnorm(1, mu, sigma) # should be using the full distribution not just the mean to do this, but then I have to use the raw data for predictions because otherwise sim_cent_cov is the wrong dimensions in defining Sigma

for(i in 2:nrow(params)) {
  mu <- params$beta_age[i]*pred_data$age_std + params$intercept[i] + as.numeric(rand_window[i,pred_data$window]) + as.numeric(rand_node[i,pred_data$node])
  sigma <- sim_cent_cov + diag(rep(params$sigma[i], n_data))
  pred_data$prediction <- mu#MASS::mvrnorm(1, mu, sigma)
  predictions <- rbind(predictions, pred_data)
  if(i %% 100 == 0) { print(i) }
}
rm(mu, sigma, pred_data, i,j,k) ; gc()
