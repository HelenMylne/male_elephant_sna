...
## define PDF output
pdf('../outputs/mpnp_nodalregression_priorcheck.pdf')
...
rm(counts_df, adj_mat, edges, ele_obs, obs, summary, make_edgelist, plot_network_threshold_mpnp) ; gc()
...
## run model
fit_mpnp1_eigen <- sampling(nodal_regression,
                           data = eigen_list,
                           cores = n_chains,
                           chains = n_chains)
...
save.image('mpnp_nodalregression/mpnpshort1_nodalregression_conditionaledge_rstan.RData')

### posterior check ####
# load('mpnp_nodalregression/mpnpshort1_nodalregression_conditionaledge_rstan.RData')
## traceplot linear effect size
traceplot(fit_mpnp1_eigen, pars = c('beta_age','sigma','predictor[1]','predictor[2]','predictor[3]','predictor[4]','predictor[5]','predictor[6]','predictor[7]','predictor[8]','predictor[9]','predictor[10]'))

## posterior predictive check
params <- rstan::extract(fit_mpnp1_eigen)
...
### predict from model ####
(summary <- as.data.frame(round(summary(fit_mpnp1_eigen)$summary[1:2, c(1, 4, 8)], 3)))
...
ggsave(filename = '../outputs/mpnpshort1_nodalregression_raw.png', device = 'png',
       plot = last_plot())
