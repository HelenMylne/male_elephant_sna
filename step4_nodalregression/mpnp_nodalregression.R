...
df_agg_mpnp5 <- read_delim('../data_processed/not yet assimilated into github version/mpnp/mpnp_period5_pairwiseevents_22.04.23.csv', delim = ',')
df_agg_mpnp5 <- df_agg_mpnp5[,c(1:16,23:26)] %>% distinct()
...
mpnp5 <- readRDS('../data_processed/not yet assimilated into github version/mpnp/mpnp5_bayesian_edgedistributions_a2.b2_period5_22.05.30.rds')
mpnp5 <- mpnp5[,c(2:ncol(mpnp5))]
mpnp5 <- mpnp5[, which(colnames(mpnp5) %in% df_agg_mpnp5$dyad)]
...
nodes1 <- df_agg_mpnp5 %>% select(id_1, age_mid_round_1) %>% distinct()
nodes2 <- df_agg_mpnp5 %>% select(id_2, age_mid_round_2) %>% distinct()
...
plot(eigen_samples_mpnp5_std ~ btwn_samples_mpnp5_std) # don't worry that high eigenvector doesn't necessarily equate to high betweenness -- https://datasciencegenie.com/what-is-centrality-in-graphs/
...
abline(h = range(eigen_mu_mpnp), lty = 2)
for(i in 1:100){
...
#### fit the model ####
colnames(mpnp5_ages) <- node_ages_mpnp5$id # check this shouldn't be as.integer(as.factor(motnp_males$id)), but 90% sure this is correct -- goes into the model in order so should come out in same order?

modeldata_ages <- as.matrix(mpnp5_ages) %>% t() # CHECK WHY DID I TRANSPOSE THIS???
...
fit_eigen_cont <- sampling(eigen_nodal_cont, data = eigen_data_mpnp5, cores = 4, chains = 4)

#### diagnostics ####
traceplot(fit_eigen_cont) # these don't look that great
...
### import data for aggregated model (binomial)
df_agg_mpnp4 <- read_delim('../data_processed/not yet assimilated into github version/mpnp/mpnp_period4_pairwiseevents_22.04.23.csv', delim = ',')
df_agg_mpnp4 <- df_agg_mpnp4[,c(1:16,23:26)] %>% distinct()
...
### load the edge weights
mpnp4 <- readRDS('../data_processed/not yet assimilated into github version/mpnp/mpnp4_bayesian_edgedistributions_a2.b2_period4_22.05.18.rds')
...
nodes1 <- df_agg_mpnp4 %>% select(id_1, age_mid_round_1) %>% distinct()
nodes2 <- df_agg_mpnp4 %>% select(id_2, age_mid_round_2) %>% distinct()
colnames(nodes1) <- c('id', 'age_mid_round')
colnames(nodes2) <- c('id', 'age_mid_round')
...
# eigenvector linear effect only
plot(NULL, xlim = c(0,60), ylim = c(-3,3), las = 1, xlab = 'age', ylab = 'mean eigenvector centrality (std)')
abline(h = range(eigen_mu_mpnp4), lty = 2)
for(i in 1:100){
...
#### fit the model ####
colnames(mpnp4_ages) <- node_ages_mpnp4$id # check this shouldn't be as.integer(as.factor(motnp_males$id)), but 90% sure this is correct -- goes into the model in order so should come out in same order?
...
### import data for aggregated model (binomial)
df_agg_mpnp3 <- read_delim('../data_processed/not yet assimilated into github version/mpnp/mpnp_period3_pairwiseevents_22.05.30.csv', delim = ',')
...
### load the edge weights
mpnp3 <- readRDS('../data_processed/not yet assimilated into github version/mpnp/mpnp3_bayesian_edgedistributions_a2.b2_period3_22.05.30.rds')
...
#### fit the model ####
colnames(mpnp3_ages) <- node_ages_mpnp3$id # check this shouldn't be as.integer(as.factor(motnp_males$id)), but 90% sure this is correct -- goes into the model in order so should come out in same order?
...
### import data for aggregated model (binomial)
df_agg_mpnp2 <- read_delim('../data_processed/not yet assimilated into github version/mpnp/mpnp_period2_pairwiseevents_22.05.30.csv', delim = ',')
...
### load the edge weights
mpnp2 <- readRDS('../data_processed/not yet assimilated into github version/mpnp/mpnp2_bayesian_edgedistributions_a2.b2_period2_22.05.30-001.rds')
...
#### fit the model ####
colnames(mpnp2_ages) <- node_ages_mpnp2$id # check this shouldn't be as.integer(as.factor(motnp_males$id)), but 90% sure this is correct -- goes into the model in order so should come out in same order?
