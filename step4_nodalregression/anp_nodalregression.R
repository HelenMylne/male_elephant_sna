...
# 
############## PLOT FINAL PLOTS -- DELETE THIS ONCE RUN #############
load('anp_nodalregression/anp_short_nodal.RData')
(final_plot <- ggplot()+
    geom_ribbon(data = newdat_summary,
                aes(ymin = predict_pred_lwr, ymax = predict_pred_upr,
    ...
    geom_point(data = nodes_all,
               ...
               aes(x = age, y = mean_eigen, colour = as.factor(window))
               )+      # original mean data points
    scale_colour_viridis_d(begin = 0, end = 0.7)+
    scale_fill_viridis_d(begin = 0, end = 0.7)+
    theme(legend.position = 'bottom')+
    labs(colour = 'time window', fill = 'time window',
         y = 'eigenvector centrality', x = 'age (years)'))
ggsave(plot = final_plot, device = 'svg', width = 800, height = 800, units = 'px',
       filename = 'anpshort_nodalregression_finalplot_nofaceting.svg',
       path = '../outputs/step4_nodalregression/')
ggsave(plot = final_plot, device = 'png', width = 800, height = 800, units = 'px',
       filename = 'anpshort_nodalregression_finalplot_nofaceting.png',
       path = '../outputs/step4_nodalregression/')

final_plot <- final_plot + facet_wrap(. ~ as.factor(window))
ggsave(plot = final_plot, device = 'svg', width = 800, height = 800, units = 'px',
       filename = 'anpshort_nodalregression_finalplot_facet.svg',
       path = '../outputs/step4_nodalregression/')
ggsave(plot = final_plot, device = 'png', width = 800, height = 800, units = 'px',
       filename = 'anpshort_nodalregression_finalplot_facet.png',
       path = '../outputs/step4_nodalregression/')
...
(final_plot <- ggplot()+
    geom_ribbon(data = newdat_summary,
                aes(x = age, fill = as.factor(window),
                    ymin = predict_pred_lwr, ymax = predict_pred_upr),
                alpha = 0.2)+
    geom_ribbon(data = newdat,
                aes(x = age, fill = as.factor(window),
                    ymin = predict_mean_lwr, ymax = predict_mean_upr),
                alpha = 0.4)+
    # geom_point(data = cents_all_df,
    #            aes(x = age, y = eigen, colour = as.factor(window)),
    #            alpha = 0.01, size = 0.5)+
    geom_line(data = newdat,
              aes(x = age, y = predict_mean, colour = as.factor(window)))+
    geom_point(data = nodes_all,
               #size = 0.5, colour = 'white',
               aes(x = age, y = mean_eigen, colour = as.factor(window))
               )+
    ...
    labs(colour = 'time window', fill = 'time window',
         y = 'eigenvector centrality', x = 'age (years)'))
ggsave(plot = final_plot, device = 'svg', width = 800, height = 800, units = 'px',
       filename = 'anplong_nodalregression_finalplot_nofaceting.svg',
       path = '../outputs/step4_nodalregression/')
ggsave(plot = final_plot, device = 'png', width = 800, height = 800, units = 'px',
       filename = 'anplong_nodalregression_finalplot_nofaceting.png',
       path = '../outputs/step4_nodalregression/')
