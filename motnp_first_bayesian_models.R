...
counts_ls <- list(
  ...
  M = nrow(counts_df),               # Number of dyads
  dyad_ids = counts_df$dyad_id,      # Vector of dyad IDs corresponding to each observation
  event = counts_df$event_count,     # Vector of binary values (0/1, presence/absence) corresponding to each observation
...
# logit(p) = w            --> parameter defining p is w = edge weight between individuals. other additional terms might be home range overlap, but in this case all elephants have an overlap in MOTNP
...
# D <- NUMBER OF SIGHITNGS OF GROUPS WHERE THEY COULD HAVE BEEN TOGETHER -- haven't simulated this because couldn't work out how to while keeping the probability pattern...
# D is the number of times that one or other of the dyad was observed
