...
#basta_data$casename <- as.numeric(basta_data$casename)
...
Out <- basta(basta_data, studyStart = Ss,
             ...
             models = c("GO"), shape = c("bathtub")) # sometimes this gives an error that x must be numeric, but str(basta_data) shows up everything except the ID number as being numeric. I've tried it also with casename as a numeric format but it didn't like that either
str(basta_data)
